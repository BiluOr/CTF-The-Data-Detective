# 🕵️‍♂️ Data Sentinel - Analyst CTF

## 📌 Overview
**Data Sentinel** is a browser-based Capture The Flag (CTF) game specifically designed for Data Analysts. 
Players step into the shoes of a data detective operating a mock financial dashboard. 

The goal is to solve 10 progressive data-driven missions by identifying specific transaction UUIDs, anomalies, and performance metrics.

## 🚀 Features
* **Interactive Dashboard:** Built-in KPIs, a dynamic data table, and interactive charts.
* **10 Progressive Levels:** Missions range from simple data isolation to complex aggregations and anomaly detection.
* **Dynamic Data Generation:** Generates a randomized dataset on load, while injecting specific deterministic targets to guarantee the answers to the missions.

## 🧠 Skills Required to Solve the CTF
To successfully crack the missions, you will need a combination of:
* **Filtering:** Using the built-in filters to isolate specific segments of the data.
* **Chart Analysis:** Observing and learning from the visual graph to identify trends.
* **Raw Data Examination:** Inspecting the raw data table to find specific UUIDs, detect anomalies, and calculate metrics.

-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# 🕵️‍♂️ משחק CTF לאנליסטים

## 📌 תקציר
משחק "תפוס את הדגל" (CTF) מבוסס דפדפן, המיועד במיוחד לאנליסטים של נתונים, השחקנים נכנסים לנעליו של בלש נתונים המפעיל דשבורד פיננסי. 

המטרה היא לפתור 10 משימות חקר נתונים ברמת קושי עולה, על מנת למצוא מזהי עסקאות (UUID), חריגות במערכת ומדדי ביצוע.

## 🚀 פיצ'רים מרכזיים
* **דשבורד אינטראקטיבי:** כולל כרטיסיות מדדים (KPIs), טבלת נתונים דינמית וגרפים מתעדכנים.
* **10 שלבים מאתגרים:** המשימות נעות מבידוד נתונים בסיסי ועד לחישובי אגרגציות מורכבים וחיפוש חריגות.
* **יצירת נתונים דינמית:** המערכת מייצרת דאטה-סט רנדומלי בכל טעינה, אך מזריקה לתוכו עסקאות מטרה ספציפיות כדי להבטיח את התשובות למשימות.

## 🧠 מיומנויות נדרשות לפתרון
כדי לפתור את החידות בהצלחה נדרש שילוב של:
* **סינון:** שימוש במסננים השונים כדי לבודד את קבוצות הנתונים הרלוונטיות.
* **הסתכלות ולמידה מהגרף:** ניתוח המגמות המוצגות בגרף כדי להבין את התמונה הגדולה ולזהות נקודות מפתח.
* **בחינת הנתונים הגולמיים:** ירידה לפרטים בטבלת הנתונים כדי למצוא מזהים ספציפיים (UUID), לאתר חריגות ולבצע חישובים מדויקים.
